...
# This application use MSIAfterburner.NET.dll by Nick Сonnors, which need MSI Afterburner to be launched
# (You can get MSIAfterburner.NET.dll here: https://forums.guru3d.com/threads/msi-afterburner-net-class-library.339656/)
# OR OpenHardwareMonitorLib.dll by Michael Möller, which you can find here:
# https://openhardwaremonitor.org/
# check for new versions for new CPU's support
#
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Fan Control 2.1")
...
class SettingsWindow(QWidget):
    ...
    def __init__(self):
        ...
        "Раздел описания элементов вкладки настройки вентиляторов"
        ...
        self.lineEdit_8.setInputMask('9' * 2)
        ...
        self.gridLayout_7.addWidget(self.label_20, 6, 0, 1, 3)
        self.gridLayout_7.addWidget(self.lineEdit_6, 0, 1, 1, 1)
        self.gridLayout_7.addWidget(self.lineEdit_8, 5, 1, 1, 1)
        self.gridLayout_7.addWidget(self.radioButton, 2, 1, 1, 2)
        self.gridLayout_7.addWidget(self.radioButton_2, 3, 1, 1, 2)
        self.gridLayout_7.addWidget(self.pushButton_15, 8, 0, 1, 3)
        self.gridLayout_7.addWidget(self.pushButton_16, 0, 2, 1, 1)
        self.gridLayout_7.addWidget(self.pushButton_17, 5, 2, 1, 1)
        self.tabWidget.addTab(self.tab_4, "")
        self.label_19.setText("Библиотека-источник:")
        self.label_20.setText("Если в режиме работы с OpenHardwareMonitorLib не отображаются часть температур - необходимо установить галку \"Запускать от имени администратора\" в свойствах совместимости!")
        self.radioButton_2.setText("OpenHardwareMonitor")
        self.radioButton.setText("MSI Afterburner")
...
class AppSettings:
    ...
    def __init__(self):
        ...
        self.settings = dict(lang='ru', com='', line_1=True, line_2=False, line_3=False, line_4=False,
                             ...
                             autoload_delay = 5, source_lib = 'ohm')
        self.cl = '1'
        ...
        self.lang_string_ru = ['Мониторинг', 'Настройки', 'Порт:', 'Подкл.', 'Язык:', 'Автозапуск приложения',
                               ...
                               'Запущенный процесс MSI Afterburner не найден!\nПереключить источник на OpenHardwareMonitorLib?', 'Частота, kHz:', 'Стоп',
                               ...
                               'Отсрочка автозагрузки:', 'Библиотека-источник:', 'OpenHardwareMonitor', 'MSI Afterburner',
                               'Если в режиме работы с OpenHardwareMonitorLib не отображаются часть температур - необходимо установить галку \"Запускать от имени администратора\" в свойствах совместимости!']
        self.lang_string_en = ['Monitoring', 'Settings', 'Port:', 'Connect', 'Lang:', 'Start with Windows',
                               ...
                               'Autoload delay:', 'Source library:', 'OpenHardwareMonitor', 'MSI Afterburner',
                               'If in OpenHardwareMonitor mode some temperatures are not displayed you need to set "Run by administrator" option in compatibility properties!']
        self.lang_string = []


    def save(self):
        """Функция сохраняющая настройки программы в файл"""
        with open(fc_path + 'fan_control.cfg', 'wb') as f:
    ...
    def load(self):
        ...
        if not os.path.isfile(fc_path + 'fan_control.cfg'):  # Проверяем существования файла конфигурации
            ...
            new_settings = ''
            try:
                with open(fc_path + 'fan_control.cfg', 'rb') as r:  # Если файл конфигурации существует читаем настройки
        ...
        settings_window.lineEdit_8.setText(str(self.settings['autoload_delay']))
        ...
        if self.settings['source_lib'] == 'msi':
            settings_window.radioButton.setChecked(True)
        else:
            settings_window.radioButton_2.setChecked(True)
            settings_window.lineEdit_8.setDisabled(True)
            settings_window.label_18.setDisabled(True)
            if processes_count('MSIAfterburner') == 0:
                settings_window.radioButton.setDisabled(True)
    ...
    def save_aditional(self):
        ...
        if not boost_zone:
            ...
            boost_zone = '90'
            settings_window.lineEdit_6.setText(boost_zone)
        if not settings_window.lineEdit_8.text():
            settings_window.lineEdit_8.setText('1')
        auto_load_delay = int(settings_window.lineEdit_8.text())
        if settings_window.radioButton.isChecked():
            source_lib = 'msi'
        else:
            source_lib = 'ohm'

        if self.settings['source_lib'] != source_lib:
            lib_changed = True

        if self.settings['autoload_delay'] != auto_load_delay:
            ad_changed = True

        self.settings.update({'boost_zone':boost_zone, 'autoload_delay':auto_load_delay, 'source_lib':source_lib})
        ...
        if lib_changed:
            ...
            QApplication.quit()
        elif ad_changed and 'FanControl' in autoload_values():
            autoload_checkbox()
    ...
    def set_lang(self):
        ...
        settings_window.pushButton_17.setText(self.lang_string[42])
        ...
        settings_window.label_18.setText(self.lang_string[46])
        settings_window.label_19.setText(self.lang_string[47])
        settings_window.label_20.setText(self.lang_string[50])
        settings_window.radioButton.setText(self.lang_string[49])
        settings_window.radioButton_2.setText(self.lang_string[48])
...
class TempsDataSource:
    ...
    def __init__(self, type):
        ...
        if type == 'msi':
            ...
            clr.AddReference(fc_path + 'OpenHardwareMonitorLib.dll')  # Добавляем пространство имён OpenHardwareMonitor
            from OpenHardwareMonitor import Hardware  # Импортируем библиотеку
            self.ohm = Hardware.Computer()
            self.ohm.CPUEnabled = True
    ...
    def get_values(self, fr=0):
        ...
        gpu_temperatures, cpu_temperatures = [], []
        ...
        if self.type == 'msi':
            ...
            try:
                ...
                if fr:
                    answer = QMessageBox.critical(ui, appSettings.lang_string[21], appSettings.lang_string[22], QMessageBox.Ok, QMessageBox.Cancel)
                    if answer == QMessageBox.Ok:
                        appSettings.settings['source_lib'] = 'ohm'
                        appSettings.save()
                        os.spawnv(os.P_NOWAIT, fc_path + 'fan_control_reset.exe', ['-'])
                        QApplication.quit()
                else:
            ...
            try:
                for device in self.ohm.Hardware:
                    device.Update()
                    ...
                    for sensor in device.Sensors:
                        if sensor.SensorType == 2:
                            ...
                            if sensor.Name.find('GPU') > -1:
                                if sensor.Name.startswith('GPU Core'):
                                    gpu_temperatures.append(round(sensor.Value))
                            else:
                                if sensor.Name.find('Package') > -1:
                                    package_temp = round(sensor.Value)
                                elif sensor.Name.find('Core') > -1:
                                    cpu_temperatures.append(round(sensor.Value))

                            if len(cpu_temperatures) == 0 and package_temp != '':
                                cpu_temperatures.append(package_temp)

                if len(gpu_temperatures) > 0 or len(cpu_temperatures) > 0:
                    return gpu_temperatures, cpu_temperatures
                else:
                    return False
...
class Processing(QThread):
    ...
    def command_preparing(self):
        ...
        self.ab = TempsDataSource(appSettings.settings['source_lib'])
        control_for = ''
...
def testing_func(duty):
    ...
    settings_window.comboBox.setDisabled(True)
    ...
    while sp.busy:
        time.sleep(1)
    settings_window.comboBox.setEnabled(True)
...
def autoload_checkbox():
    ...
    if ui.checkBox.isChecked():
        if 'FanControl' not in values_list:
            ...
            if (appSettings.settings['source_lib'] == 'msi'):
                winreg.SetValueEx(as_key, 'FanControl', 0, 1, '"{0}" -{1}'.format(executable, appSettings.settings['autoload_delay']))
            else:
                winreg.SetValueEx(as_key, 'FanControl', 0, 1, '"{0}"'.format(executable))
...
def donate_button():
    ...
    if appSettings.settings['lang'] == 'ru':
        webbrowser.open_new_tab('https://yoomoney.ru/to/41001350655590')
    else:
        webbrowser.open_new_tab('https://yoomoney.ru/to/41001350655590')


def change_sourcelib_delay():
    """Обработка нажатия кнопки смены отсрочки автозапуска"""
    if (settings_window.radioButton.isChecked()):
        if not settings_window.lineEdit_8.text() or not settings_window.lineEdit_8.text().isdecimal():
            settings_window.lineEdit_8.setText('1')
    appSettings.save_aditional()

def source_button_msi():
    """Обработка нажатия ради-кнопки выбора библиотеки-источника"""
    settings_window.radioButton_2.setChecked(False)
    settings_window.label_18.setDisabled(False)
    settings_window.lineEdit_8.setDisabled(False)

def source_button_ohm():
    """Обработка нажатия ради-кнопки выбора библиотеки-источника"""
    settings_window.radioButton.setChecked(False)
    settings_window.label_18.setDisabled(True)
    settings_window.lineEdit_8.setDisabled(True)
...
def processes_count(name):
    """Поиск колличества запущенных процессов с заданым именем (используется для поиска запущенных инстанций и MSIAB)"""
...
if processes_count('fan_control') < 3:  # Инициализируем программу при отсутствии уже запущенных копий
    ...
    appSettings.load()
    tds = TempsDataSource(appSettings.settings['source_lib']);
    if tds.mon_conf():  # Инициализируем программу если удалось подключиться к MSI:AB
        del tds
        ...
        settings_window.pushButton_7.clicked.connect(lambda: donate_button())
        settings_window.pushButton_11.clicked.connect(lambda: about_button())
        ...
        settings_window.pushButton_17.clicked.connect(lambda: change_sourcelib_delay())
        settings_window.radioButton.clicked.connect(lambda: source_button_msi())
        settings_window.radioButton_2.clicked.connect(lambda: source_button_ohm())
